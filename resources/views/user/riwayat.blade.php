...
<div class="content">
    ...
    @if($riwayat->count() > 0)
        ...
        <div class="trx-card">
            <div class="trx-head">
                <div>
                    <div class="trx-id">#PJL-{{ str_pad($r->PenjualanID, 3, '0', STR_PAD_LEFT) }}</div>
                    <div class="trx-date">{{ \Carbon\Carbon::parse($r->TanggalPenjualan)->isoFormat('D MMMM Y') }}</div>
                </div>
                <div class="trx-total">Rp {{ number_format($r->TotalHarga, 0, ',', '.') }}</div>
            </div>
            <table>
                <thead>
                    <tr>
